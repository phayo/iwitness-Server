...
class IwitnessKafkaResourceIT {
    ...
    void setup() {
        ...
        IwitnessKafkaResource kafkaResource = new IwitnessKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/iwitness-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/iwitness-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
