...
export const ACTION_TYPES = {
  SEARCH_DEVICES: 'device/SEARCH_DEVICES',
...
export const getSearchEntities: ICrudSearchAction<IDevice> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_DEVICES,
  payload: axios.get<IDevice>(`${apiSearchUrl}?query=${query}`),
});
...
export const partialUpdate: ICrudPutAction<IDevice> = entity => async dispatch => {
  const result = await dispatch({
